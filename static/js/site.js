...
(() => {
  ...
  /* ---------- Framework: code-descent progress + rail spy ----------
     Mirrors Launch Control, but descends into the engine. The floating
     Rust-code backdrop (.fw-code in framework.html) is CSS, driven by the
     three scroll custom properties written here: --fw-depth settles the
     scene in over the first fifth of scroll, --fw-scroll parallaxes the code
     layers the whole way down, and --fw-end reveals the molten bedrock over
     the final stretch. The reading column reuses the `lc-` journey classes. */
  if (document.body.classList.contains("fw")) {
    ...
    const updateDive = () => {
      ...
      if (bar) bar.style.width = (p * 100).toFixed(1) + "%";
      const t = Math.min(1, p / 0.2);
      root.style.setProperty("--fw-depth", (t * t * (3 - 2 * t)).toFixed(3));
      root.style.setProperty("--fw-scroll", p.toFixed(4));
      const er = Math.min(1, Math.max(0, (p - 0.8) / 0.2));
      root.style.setProperty("--fw-end", (er * er * (3 - 2 * er)).toFixed(3));
    };
    window.addEventListener("scroll", updateDive, { passive: true });
    window.addEventListener("resize", updateDive);
    updateDive();

    // Descent rail: highlight the current chapter, mark passed ones done.
    const items = Array.from(document.querySelectorAll(".lc-rail-item"));
    const ids = items.map((a) => a.getAttribute("href").slice(1));
